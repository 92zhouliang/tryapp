	...
	<div class="content-page uk-body">
		...
		<div class="calendar">
			...
			<table class="sign_tab" border="0px" cellpadding="0px" cellspacing="0px">
				...
				<tbody>
					<tr v-for="(item,index) in dateArr" v-if="contains(item)" :key="index">
						<td v-for="(data,index) in item" :class="{ 'cur_day': data == date, 'ui-state-default': true }" v-if="isCheck(data)" :key="index">
						</td>
						<td v-else :class="{ 'over':data == '', 'cur_day': data == date }" @click='check(data)'>
							{{data}}
						</td>
					</tr>
				</tbody>
	...
	export default {
		...
		methods: {
			...
			nextMonth() {
                ...
                this.getCalendar();
                console.log(this.getCalendarTime());
			},
			prevMonth() {
				if(this.month == 1) {
					this.year --;
...
<style type="text/css" scoped>
	...
	.calendar button.month-less {left: 20px;background: url('./../../assets/images/left-icon.png') no-repeat left -60px;}
	.calendar button.month-add {right: 20px;background: url('./../../assets/images/right-icon.png') no-repeat left -60px;}
	...
	.calendar .ui-state-default{background: url('./../../assets/images/checkin.png') no-repeat center center;}
